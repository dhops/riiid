...
from RRNCF.model_users import RRNCF
...
def pad_collate(batch):
  ...
  users = torch.from_numpy(np.asarray(users))
...
def main(dataset_name, dataset_path, model_name, epoch, learning_rate,
    ...
    model = RRNCF(embed_dim=16, mlp_dim=16, dropout=0.2, questionset_size=n_item, tagset_size=n_tag, userset_size=n_users)
    model.to(device)
