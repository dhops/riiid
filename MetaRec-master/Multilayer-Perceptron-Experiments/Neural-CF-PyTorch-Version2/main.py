...
def get_dataset(name, path):
    ...
    :return: RiiidDataset
    """
    if name == 'RiiidDataset':
        return RiiidDataset(path)
    else:
...
def train(model, optimizer, data_loader, criterion, device, log_interval=1000):
    ...
    for i, (fields, target) in enumerate(tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0)):
        ...
        y = model(fields)
        loss = criterion(y, target.float().squeeze())
        model.zero_grad()
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', default='RiiidDataset')
    parser.add_argument('--dataset_path', default='../../../input/forMF/dataset.npz')
