...
class RiiidDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path, sep=',', engine='python', header=None):
        ...
        # Retrieve the items and ratings data
        self.items = np.concatenate((full_data['train_x'],full_data['test_x']),axis=0).astype(np.int)
        self.targets = np.concatenate((full_data['train_y'],full_data['test_y']),axis=0).astype(np.float32)
